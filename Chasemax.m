...
%         Iv                = imbinarize(Ivessel);
        DoS              =  mean2(Ivessel);
        Ivessel           = imnlmfilt(Ivessel,'DegreeOfSmoothing',DoS);
        Iv                = blockproc(m*Ivessel, [b b], @(block) ((block.data)), 'Border', [4 4]);
        Iv                = bwareaopen(imbinarize(Iv),x);
%         Iv                = Iv.*cm;
        res               = imbinarize(result)+(Iv);
        resul             = imclose(res,se);
        resl(:,:,i)       = resul.*double(mask);
        resl(:,:,i)       = bwareaopen(resl(:,:,i),500);
        resl(:,:,i) = imnlmfilt(resl(:,:,i),'DegreeOfSmoothing',DoS/10);
        i1 = i;
